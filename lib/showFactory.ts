...
export function emptyShow(name = "Untitled Show"): Show {
  ...
  return {
    ...
    captureDevices: [
      { id: uid("cap"), name: "NDI Source 1", nodeId: "local-runner", kind: "NDI", signal: "WATCHIN-PREVIEW" },
    ],
    variables: [
...
export function makeDemoShow(): Show {
  const show = emptyShow("WATCHIN Demo — LED Wall");
  show.displays = [
  ...
  const bars = emptyAsset({
    ...
    url: svgData(`<svg xmlns="http://www.w3.org/2000/svg" width="1920" height="1080">
      ...
      <text x="960" y="560" text-anchor="middle" fill="#fff" font-family="Arial" font-size="72">WATCHOUT 7 / WATCHIN</text>
    </svg>`),
  });
