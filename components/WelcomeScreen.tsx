    ...
    <div className="flex h-screen w-screen overflow-hidden bg-[#0d0d0d]">
      <div className="relative flex w-[58%] flex-col justify-center px-16">
        ...
        <div className="relative">
          ...
          <p className="mt-6 max-w-xl text-[13px] leading-relaxed text-stone-400">
            Compose video, stills, NDI, audio and control across unlimited displays.
            Producer authors the show. Director holds state. Runners render outputs.
            Asset Manager optimizes and distributes media.
          </p>
          <div className="mt-10 flex flex-wrap gap-3">
            <WelcomeBtn label="New Show" hint="Local Director + Asset Manager" onClick={newShow} primary />
            <WelcomeBtn label="Open Show" hint="Load a .watch.json file" onClick={() => fileRef.current?.click()} />
            <WelcomeBtn label="CONNECT" hint="Open last saved / Director" onClick={openLocal} />
            <WelcomeBtn label="Demo Show" hint="3-wide LED wall with cues" onClick={openDemo} />
            <WelcomeBtn
              label="Learn More"
              hint="WATCHOUT 7 documentation"
              onClick={() => window.open("https://docs.dataton.com/watchout-7-new/watchout/getting-started/welcome-to-watchout-7.html", "_blank")}
            />
          </div>
          <input
            ref={fileRef}
      ...
      <div className="flex w-[42%] flex-col border-l border-[#2a2a2a] bg-[#161616] px-10 py-12">
        ...
        <div className="mt-6 text-[11px] text-stone-600">
          WATCHIN is a browser recreation of Dataton WATCHOUT 7 Producer. Media stays local in this session.
        </div>
      </div>
    </div>
  );
